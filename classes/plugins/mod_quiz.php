...
class mod_quiz {
    ...
    public static function editingform($mform): void {
        ...
        $mform->addElement(
            'checkbox',
            'resetquizoverride',
            get_string('resetquizoverride', 'local_recertify')
        );
        $mform->addHelpButton('resetquizoverride', 'resetquizoverride', 'local_recertify');
        ...
        $mform->hideIf('archivequiz', 'quiz', 'noteq', LOCAL_RECERTIFY_DELETE);
        $mform->hideIf('resetquizoverride', 'quiz', 'noteq', LOCAL_RECERTIFY_DELETE);
    }
    ...
    public static function settings($settings) {
        ...
        $settings->add(new \admin_setting_configcheckbox(
            'local_recertify/resetquizoverride',
            new lang_string('resetquizoverride', 'local_recertify'),
            new lang_string('resetquizoverride_help', 'local_recertify'),
            0
        ));
    }
    ...
    public static function reset($userid, $course, $config) {
        ...
        if (empty($config->quiz)) {
            ...
            if ($config->archivequiz) {
                ...
                $DB->insert_records('local_recertify_qa', $quizattempts);
                ...
                $DB->insert_records('local_recertify_qg', $quizgrades);
            }
            if (!empty($config->resetquizoverride)) {
                // Extra attempts granted in an earlier cycle would otherwise accumulate forever.
        ...
        } else if ($config->quiz == LOCAL_RECERTIFY_EXTRAATTEMPT) {
