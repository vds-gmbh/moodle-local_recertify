 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class check_recertify extends \core\task\scheduled_task {
    /**
     * Returns the name of this task.
     */
    public function get_name() {
        // Shown in admin screens.
        return get_string('recertifytask', 'local_recertify');
    }
    ...
    protected function notify_user($userid, $course, $config) {
        ...
        if (!$config->recertifyemailenable) {
            return;
        }
        ...
        if (trim($config->recertifyemailbody) !== '') {
            ...
            $messagetext = get_string('recertifyemaildefaultbody', 'local_recertify', $a);
        ...
        email_to_user($userrecord, $from, $subject, $messagetext, $messagehtml, true);
    }
    ...
    public function reset_user($userid, $course, $config) {
        ...
        // Store reset event data for later use in supervisor notifications.
        \local_recertify\reset_log::store_reset_event_data($event);
