...
if (empty($config->enable)) {
    throw new moodle_exception('recertifynotenabled', 'local_recertify');
}
...
// Print the form.
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string("resetcompletionfor", "local_recertify", fullname($user)));

$cancelurl = course_get_url($course);
$confirmurl = $PAGE->url;
$confirmurl->param('confirm', 1);
$confirmurl->param('user', $userid);
$message = get_string("resetcompletionconfirm", "local_recertify", fullname($user));
